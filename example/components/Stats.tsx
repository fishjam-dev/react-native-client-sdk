...
export const Stats = ({stats, label, accessibilityLabel}: StatsProp) => {
  ...
  const getLimitationDurationsDataset = useCallback(
    ...
    [generateLimitationDurationInitData, stats],
  );
  ...
  const getChart = (statistic: string) => {
    return statistic === 'qualityLimitationDurations'
      ? getLineChart(getLimitationDurationsDataset(label, statistic), true)
      : getLineChart(
          [
            {
              label: statistic,
